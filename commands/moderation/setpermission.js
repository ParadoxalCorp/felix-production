...
class SetPermission extends ModerationCommands {
    ...
    async setPermission(client, message, guildEntry, args) {
        ...
        return message.channel.createMessage(`:white_check_mark: Successfully ${args.override ? 'allowed' : 'restricted'} the permission \`${args.permission}\` for the ${args.targetType === 'global' ? 'server' : args.targetType} ${args.target.name || args.target.username ? ('**' + (args.target.name || new client.structures.ExtendedUser(args.target).tag, client) + '**') : ''}`);
    }
}
