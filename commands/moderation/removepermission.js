...
class RemovePermission extends Command {
    ...
    async removePermission(client, message, guildEntry, args) {
        ...
        return message.channel.createMessage(`:white_check_mark: Successfully removed the permission \`${args.permission}\` which was ${restricted ? 'restricted' : 'allowed'} on the ${args.targetType === 'global' ? 'server' : args.targetType} ${args.target.name || args.target.username ? ('**' + (args.target.name || new client.structures.ExtendedUser(args.target).tag, client.bot) + '**') : ''}`);
    }
}
