...
class GetPermissions extends Command {
    ...
    async getPermissions(client, message, guildEntry, args) {
        ...
        if (!targetPerms) {
            return message.channel.createMessage(`:x: There is not any permissions set for the ${args.targetType} **${(args.target.name || client.structures.ExtendedUser(args.target).tag)}**`);
        }
    ...
    formatPermissions(client, message, guildEntry, args, targetPerms) {
        return {
            title: `${args.targetType === 'global' ? 'Global' : ''} Permissions ${args.targetType !== 'global' ? ('for the ' + args.targetType + ' ' + (args.target.name || client.structures.ExtendedUser(args.target).tag)) : ''}`,
